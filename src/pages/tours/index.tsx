...
import { getTours, deleteTourById } from 'apiSdk/tours';
import { TourInterface } from 'interfaces/tour';

type ColumnType = ColumnDef<TourInterface, unknown>;
...
export function TourListPage(props: TourListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TourInterface>>(
    () => `/tours?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TourInterface) => {
    if (hasAccess('tour', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/tours/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <>
          {hasAccess('tour', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/tours/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('tour', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="tour" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Tour
        </Text>
        {hasAccess('tour', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/tours/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
